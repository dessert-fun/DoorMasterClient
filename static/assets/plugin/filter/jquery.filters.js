(function ($) {
    ...
    function buildFiltersHtml() {
        ...
        if (6 > realIndex) {
            ...
            for (var i = 0; i < (6 - realIndex); i++) {
                filterInternalHtml += buildSingleParameterHtml(i + realIndex, {
                    name: '',
                    type: empty,
                    attributeName: 'bla'
                })
    ...
    function htmlForParameterOptions(parameter, filteredOptions) {
        ...
        if (selectedFilter) {
            selectedValues = $.map(selectedFilter.values, function (data) {
        ...
        $.each([textType, dateRange], function (_, inputType) {
            $('.select-parameter-box.' + inputType).each(function (_, selectBox) {
                ...
                if (0 < tempValues.length) {
                    DoNotSaveThisInTheSelectedFiltered[serverParameterName] = {
    ...
    function genericCollect(selectBox, inputType) {
        ...
        return {
            value: DoNotSaveThisInTheSelectedFiltered[serverParameterName],
            serverParameterName: serverParameterName
        }
    }
    ...
    function existsInSelectedFilter(selectFilterValues, relatedToValue) {
        ...
        selectedValues = selectFilterValues.map(function (element) {
